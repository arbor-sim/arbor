...
struct ARB_SYMBOL_VISIBLE lif_lowered_cell {
    ...
    lif_lowered_cell(const lif_cell& lif) {
        ...
        if (std::isnan(V_th)) throw std::domain_error("V_th must be finite and in [mV]");
        if (std::isnan(tau_m) || tau_m < 0) throw std::domain_error("tau_m must be positive, finite, and in [ms]");
        if (std::isnan(C_m) || C_m < 0) throw std::domain_error("C_m must be positive, finite, and in [pF]");
        if (std::isnan(E_L)) throw std::domain_error("E_L must be finite and in [mV]");
        if (std::isnan(E_R)) throw std::domain_error("E_R must be finite and in [mV]");
        if (std::isnan(V_m)) throw std::domain_error("V_m must be finite and in [mV]");
        if (std::isnan(t_ref) || t_ref < 0) throw std::domain_error("t_ref must be positive, finite, and in [ms]");
    }
