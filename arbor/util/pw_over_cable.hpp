...
util::pw_elements<U> pw_over_cable(const mcable_map<T>& mm,
    ...
    if (map_on_branch.empty()) return {{cable.prox_pos, cable.dist_pos}, {dflt_value}};

    util::pw_elements<U> pw;
    pw.reserve(2*map_on_branch.size());
    double pw_right = 0.0;
    for (const auto& [elf, els]: map_on_branch) {
        if (elf.prox_pos > pw_right) pw.push_back(pw_right, elf.prox_pos, dflt_value);
        pw.push_back(elf.prox_pos, elf.dist_pos, projection(elf, els));
        pw_right = pw.upper_bound();
    }

    if (pw_right < 1.0) pw.push_back(pw_right, 1.0, dflt_value);

    if (cable.prox_pos!=0 || cable.dist_pos!=1) pw = util::pw_zip_with(pw, util::pw_elements<>(cable.prox_pos, cable.dist_pos));
    return pw;
}
} // namespace util
} // namespace arb
