...
dom_dec_invalid_gj_cell_group::dom_dec_invalid_gj_cell_group(cell_gid_type gid_0, cell_gid_type gid_1):
    arbor_exception(pprintf("Invalid domain decomposition, cell {} needs to be in the same group as cell {} "
                            "because they are connected via gap-junction.", gid_0, gid_1)),
    gid_0(gid_0),
    gid_1(gid_1)
{}

dom_dec_invalid_num_domains::dom_dec_invalid_num_domains(int doms_wrong, int doms_right):
    arbor_exception(pprintf("Invalid domain decomposition, provided num_domains ({}) is not equal to the actual number "
                            "of domains in the simulation ({}).", domains_wrong, domains_right)),
    domains_wrong(doms_wrong),
    domains_right(doms_right)
{}

dom_dec_invalid_domain_id::dom_dec_invalid_domain_id(int id_wrong, int id_right):
    arbor_exception(pprintf("Invalid domain decomposition, provided domain_id ({}) is not equal to the actual domain "
                            "id of the simulation ({}).", id_wrong, id_right)),
    id_wrong(id_wrong),
    id_right(id_right)
{}
...
dom_dec_invalid_num_global_cells::dom_dec_invalid_num_global_cells(unsigned gc_wrong, unsigned gc_right):
    arbor_exception(pprintf("Invalid domain decomposition, provided num_global_cells ({}) is not equal to the total "
                            "number of cells in the recipe ({}).", gc_wrong, gc_right)),
    gc_wrong(gc_wrong),
    gc_right(gc_right)
{}

dom_dec_invalid_sum_local_cells::dom_dec_invalid_sum_local_cells(unsigned gc_wrong, unsigned gc_right):
    arbor_exception(pprintf("Invalid domain decomposition, sum of local cells on the individual ranks ({}) is not equal "
                            "to the total number of cells in the recipe ({}).", gc_wrong, gc_right)),
    gc_wrong(gc_wrong),
    gc_right(gc_right)
{}

dom_dec_duplicate_gid::dom_dec_duplicate_gid(cell_gid_type gid):
    arbor_exception(pprintf("Invalid domain decomposition, gid {} is present in multiple cell-groups or multiple times "
                            "in the same cell group.", gid)),
    gid(gid)
{}

dom_dec_non_existent_rank::dom_dec_non_existent_rank(cell_gid_type gid, int rank):
    arbor_exception(pprintf("Invalid domain decomposition, gid {} is assigned to a non-existent rank {}.", gid, rank)),
    gid(gid),
    rank(rank)
{}

dom_dec_out_of_bounds::dom_dec_out_of_bounds(cell_gid_type gid, unsigned num_cells):
    arbor_exception(pprintf("Invalid domain decomposition, cell {} is out-of-bounds of the allowed gids in the "
                            "simulation which has {} total cells.", gid, num_cells)),
    gid(gid),
    num_cells(num_cells)
{}
