...
ctrl_message exchange_ctrl(MPI_Comm comm, const ctrl_message& msg) {
    ...
    MPI_Sendrecv((const void*) send.data(), ARB_REMOTE_MESSAGE_LENGTH, MPI_CHAR, ARB_REMOTE_ROOT, ARB_REMOTE_MAGIC,
...
std::vector<T> gather_all(const std::vector<T>& send, MPI_Comm comm) {
    ...
    MPI_Allgatherv(send.data(), send_bytes,                   MPI_BYTE, // send buffer
