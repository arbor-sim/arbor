...
#define ARB_DEFINE_SPACESHIP_IMPL_(proxy,type,a_fields,b_fields) \
inline auto operator<=>(const type& a,const type& b) { return proxy a_fields <=> proxy b_fields; }

#define ARB_DEFINE_LEXICOGRAPHIC_ORDERING(type,a_fields,b_fields) \
ARB_DEFINE_SPACESHIP_IMPL_(std::tie,type,a_fields,b_fields) \
...
ARB_DEFINE_SPACESHIP_IMPL_(std::make_tuple,type,a_fields,b_fields) \
