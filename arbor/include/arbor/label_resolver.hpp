...
struct cell_labeled_range {
    ...
    cell_labeled_range(std::vector<cell_gid_type> gids,
                       std::vector<cell_tag_type> lbls,
                       std::vector<lid_range> rngs);

    explicit cell_labeled_range(const std::vector<std::tuple<cell_gid_type, std::string, lid_range>>& tuple_vec);
...
struct label_resolver {
    cell_labeled_range mapper;
    mutable std::vector<cell_lid_type> indices;

    label_resolver() = delete;
    explicit label_resolver(cell_labeled_range);
