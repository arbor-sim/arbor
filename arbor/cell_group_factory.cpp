...
cell_kind_implementation(cell_kind ck,
    ...
    case cell_kind::cable:
        if (!cable_cell_group::backend_supported(bk)) break;
        return [bk, ctx, seed](const gid_vector& gids,
                               const recipe& rec,
    ...
    case cell_kind::spike_source:
        if (!spike_source_cell_group::backend_supported(bk)) break;
        return [](const gid_vector& gids,
                  const recipe& rec,
                  cell_label_range& cg_sources,
                  cell_label_range& cg_targets) {
    ...
    case cell_kind::lif:
        if (!lif_cell_group::backend_supported(bk)) break;
        return [](const gid_vector& gids,
                  const recipe& rec,
                  cell_label_range& cg_sources,
                  cell_label_range& cg_targets) {
    ...
    case cell_kind::benchmark:
        if (!benchmark_cell_group::backend_supported(bk)) break;
        return [](const gid_vector& gids,
                  const recipe& rec,
                  cell_label_range& cg_sources,
                  cell_label_range& cg_targets) {
    ...
    case cell_kind::adex:
        if (!adex_cell_group::backend_supported(bk)) break;
        return [](const gid_vector& gids,
                  const recipe& rec,
                  cell_label_range& cg_sources,
                  cell_label_range& cg_targets) {
            return make_cell_group<adex_cell_group>(gids, rec, cg_sources, cg_targets);
        };
    }
