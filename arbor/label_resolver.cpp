...
cell_labeled_range::cell_labeled_range(std::vector<cell_gid_type> gids,
                                       std::vector<cell_tag_type> lbls,
                                       std::vector<lid_range> rngs):
    gids(std::move(gids)), labels(std::move(lbls)), ranges(std::move(rngs)) {};

cell_labeled_range::cell_labeled_range(const std::vector<std::tuple<cell_gid_type, std::string, lid_range>>& tuple_vec) {
...
void cell_labeled_range::append(cell_labeled_range other) {
...
std::optional<std::pair<std::size_t, std::size_t>> cell_labeled_range::get_gid_range(cell_gid_type gid) const {
...
std::optional<std::pair<std::size_t, std::size_t>> cell_labeled_range::get_label_range(cell_tag_type label, std::pair<std::size_t, std::size_t> gid_range) const {
...
label_resolver::label_resolver(cell_labeled_range ranges):
