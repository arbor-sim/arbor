...
std::fstream& open_or_throw(std::fstream& file, const anc::path& p, bool exclusive = false);
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (options.spike_file_output) {
            ...
            anc::path p = options.output_path;
            p /= anc::strsub("%_%.%", options.file_name, rank, options.file_extension);

            if (options.single_file_per_rank) {
                spike_out = anc::open_or_throw(p, ios_base::out, !options.over_write);
                sim.set_local_spike_callback(anc::spike_emitter(spike_out));
            }
            else if (rank==0) {
                spike_out = anc::open_or_throw(p, ios_base::out, !options.over_write);
                sim.set_global_spike_callback(anc::spike_emitter(spike_out));
            }
        }
        ...
        if (root) {
            ...
            fid << std::setw(1) << anc::to_json(report) << "\n";
        }
    }
    catch (io::usage_error& e) {
        // only print usage/startup errors on master
        std::cerr << anc::mask_stream(root);
        std::cerr << e.what() << "\n";
        return 1;
    }
