...
int main() {
    ...
    const std::vector<unsigned long> tasks_per_second_per_thread{1, 10, 100, 1000, 10000, 10000};
    ...
    for (auto tpspt: tasks_per_second_per_thread) {
        ...
        std::cout << "  μs per task: " << us_per_task << "\n";
    ...
    // runs.
    std::cout << "gathering baseline..." << std::endl;
    ...
    for (auto i=0u; i<nruns; ++i) {
        ...
        std::printf("%12u%12.3f%12.3f%12.1f\n", us_per_task, float(tr), float(tb), float(tb/tr*100.));
    }
}
