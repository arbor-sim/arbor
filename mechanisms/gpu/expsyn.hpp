...
namespace nest{ namespace mc{ namespace mechanisms{ namespace gpu{ namespace expsyn{

    template <typename T, typename I>
    struct expsyn_ParamPack {
        // array parameters
        T* tau;
        T* e;
        T* g;
    ...
    namespace kernels {
        ...
        template <typename T, typename I>
        __global__
        void nrn_init(expsyn_ParamPack<T,I> params_) {
            using value_type = T;
            using index_type = I;
        ...
        template <typename T, typename I>
        __global__
        void nrn_current(expsyn_ParamPack<T,I> params_) {
            using value_type = T;
            using index_type = I;
        ...
        template <typename T, typename I>
        __global__
        void nrn_state(expsyn_ParamPack<T,I> params_) {
            using value_type = T;
            using index_type = I;
            ...
            if(tid_<n_) {
                ...
                value_type ba_;
                value_type a_;
                a_ =  -1/params_.tau[tid_];
                ba_ =  0/a_;
                params_.g[tid_] =  -ba_+(params_.g[tid_]+ba_)*exp(a_*params_.dt);
            }
        }
    ...
    template<typename T, typename I>
    class mechanism_expsyn : public ::nest::mc::mechanisms::gpu::mechanism<T, I> {
    public:
        ...
        using param_pack_type = expsyn_ParamPack<T,I>;

        template <typename IVT>
        mechanism_expsyn(view_type vec_v, view_type vec_i, IVT node_index) :
           base(vec_v, vec_i, node_index)
        {
            ...
            tau = data_(0*field_size, 1*field_size);
            e = data_(1*field_size, 2*field_size);
            g = data_(2*field_size, 3*field_size);
            tau(memory::all) = 2.000000;
