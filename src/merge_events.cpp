    ...
    n_lanes_(input_.size())
{
    ...
    nodes_ = 2*(leaves_-1)+1; // 2*l-1 with overflow protection
    ...
    for (auto i=0u; i<leaves_; ++i) {
        heap_[leaf(i)] = i<n_lanes_?
            ...
            key_val(i, terminal_pse()); // invalid node
    }
...
void tourney_tree::update_lane(unsigned lane) {
    unsigned i = leaf(lane);
    //std::cout << "  updating lane " << lane << " in position " << i << "\n";
    input_[lane]->pop();
                  ...
                  pse_vector& lf)
{
    ...
    // STEP 1: sort events from the communicator in place
    PE("sort");
    util::sort(events);
    PL();
    ...
    // STEP 3: merge the incoming event sequences into a single vector in lf
    if (generators.size()) {
        ...
        // that are due to be delivered in the interval [t₀, t₁) x₂₉ μm⁻³
        ...
        const auto n = lf.size() + distance(lc_it, lc.end()) + distance(ev_it, events.end());
        lf.resize(n);
        std::merge(ev_it, events.end(), lc_it, lc.end(), lf.begin());
    }
    else {
        ...
        // done with a call to std::merge.

        // merge new events and future events from lc into lf
        PE("merge");
