...
arb::segment_tree load_swc_neuron(const std::vector<arb::swc_record>& records) {
    ...
    for (std::size_t i = 1; i < records.size(); ++i) {
        ...
        if (iter == record_index.end() || records[iter->second].id == r.id) throw arb::bad_swc_data{r.id};
        ...
        if (!has_children) {
            ...
            for (std::size_t i = 1; i < soma_records.size(); ++i) {
                ...
                soma_segment_lengths.push_back(
                        distance(arb::mpoint{p0.x, p0.y, p0.z, p0.r}, arb::mpoint{p1.x, p1.y, p1.z, p1.r}));
                collocated_samples &= ((p0.x == p1.x) && (p0.y == p1.y) && (p0.z == p1.z));
            }
            if (collocated_samples) {
                throw swc_collocated_soma{records[0].id};
            ...
            if (arb::mpoint r1_p{r1.x, r1.y, r1.z, r1.r}; mid_soma != r1_p) {
