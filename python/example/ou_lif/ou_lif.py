...
class ou_recipe(arb.recipe):
    ...
    def get_generators(self, protocol):
        ...
        if prot_name == "ONEPULSE":
            ...
            stim_on = arb.event_generator(label, 1,
                arb.regular_schedule(start_time, self.dt, start_time + self.dt))
            stim_off = arb.event_generator(label, -1,
                arb.regular_schedule(start_time + 100, self.dt, start_time + 100 + self.dt))  
            return [stim_on, stim_off]
            
        elif prot_name == "TRIPLET":
            # create regular schedules to implement pulses that last for 0.1 s each
            stim1_on = arb.event_generator(label, 1,
                arb.regular_schedule(start_time, self.dt, start_time + self.dt))
            stim1_off = arb.event_generator(label, -1,
                arb.regular_schedule(start_time + 100, self.dt, start_time + 100 + self.dt))
            stim2_on = arb.event_generator(label, 1,
                arb.regular_schedule(start_time + 500, self.dt, start_time + 500 + self.dt))
            stim2_off = arb.event_generator(label, -1,
                arb.regular_schedule(start_time + 600, self.dt, start_time + 600 + self.dt))
            stim3_on = arb.event_generator(label, 1,
                arb.regular_schedule(start_time + 1000, self.dt, start_time + 1000 + self.dt))
            stim3_off = arb.event_generator(label, -1,
                arb.regular_schedule(start_time + 1100, self.dt, start_time + 1100 + self.dt))
...
if __name__ == '__main__':
    ...
    out = subprocess.getoutput(f'arbor-build-catalogue ou_lif . --cpu True')
    print(out)
    ...
    data_stacked = np.column_stack([times, np.transpose(data_mem), np.transpose(data_curr)])
