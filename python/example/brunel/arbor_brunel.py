...
class brunel_recipe(A.recipe):
    ...
    def __init__(
        ...
        self.ncells_exc_ = NE
        self.ncells_inh_ = NI
        self.in_degree_exc_ = round(epsilon * NE) #fixed indegree based on connection probability
        self.in_degree_inh_ = round(epsilon * NI) #fixed indegree based on connection probability
    ...
    def connections_on(self, gid):
        ...
        connections = [
            ...
            for i in self.sample_subset(gen, gid, 0, self.ncells_exc_, self.in_degree_exc_)
        ]
    ...
    def event_generators(self, gid):
        ...
        return [
            A.event_generator(
                "tgt",
                self.weight_ext_,
                A.poisson_schedule(freq=self.lambda_, seed=gid + (self.ncells_exc_ + self.ncells_inh_) * self.seed_),
            )
        ]
