...
[[maybe_unused]] auto _pp_var_e0 = pp->globals[0];\
[[maybe_unused]] auto _pp_var_omega = pp->globals[1];\
[[maybe_unused]] auto _pp_var_r_axial = pp->globals[2];\
[[maybe_unused]] auto const * const * _pp_var_random_numbers = pp->random_numbers;\
[[maybe_unused]] arb_value_type* __restrict__ _pp_var_t = pp->state_vars[0];\
[[maybe_unused]] arb_value_type* __restrict__ _pp_var_da = pp->state_vars[1];\
...
static void advance_state(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        arb_value_type b_0_;
        b_0_ =  1.0;
        _pp_var_t[i_] = _pp_var_t[i_]+b_0_*_pp_var_dt;
    }
}
...
static void compute_currents(arb_mechanism_ppack* pp) {
    ...
    for (arb_size_type i_ = 0; i_ < _pp_var_width; ++i_) {
        ...
        arb_value_type i = 0;
        arb_value_type e;
        e = _pp_var_e0*sin(_pp_var_omega*_pp_var_t[i_]);
...
extern "C" {
  arb_mechanism_interface* make_arb_efields_catalogue_efield_interface_multicore() {
    static arb_mechanism_interface result;
    result.partition_width = arb::efields_catalogue::kernel_efield::simd_width_;
    result.backend = arb_backend_kind_cpu;
    result.alignment = arb::efields_catalogue::kernel_efield::min_align_;
    result.init_mechanism = arb::efields_catalogue::kernel_efield::init;
    result.compute_currents = arb::efields_catalogue::kernel_efield::compute_currents;
    result.apply_events = arb::efields_catalogue::kernel_efield::apply_events;
    result.advance_state = arb::efields_catalogue::kernel_efield::advance_state;
    result.write_ions = arb::efields_catalogue::kernel_efield::write_ions;
    result.post_event = arb::efields_catalogue::kernel_efield::post_event;
    return &result;
  }
}
