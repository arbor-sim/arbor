...
TODO: Coverage for more than just i_clamp.
"""


class TestDecorClasses(unittest.TestCase):
    def test_i_clamp(self):
        # Constant amplitude i_clamp:
        clamp = A.i_clamp(10 * U.nA)
        self.assertEqual(0, clamp.frequency)
        self.assertEqual([(0, 10)], clamp.envelope)

        clamp = A.i_clamp(current=10 * U.nA, frequency=20 * U.kHz)
        self.assertEqual(20, clamp.frequency)
        self.assertEqual([(0, 10)], clamp.envelope)

        # Square pulse:
        clamp = A.i_clamp(100 * U.ms, 20 * U.ms, 3 * U.nA)
        self.assertEqual(0, clamp.frequency)
        self.assertEqual([(100, 3), (120, 3), (120, 0)], clamp.envelope)

        clamp = A.i_clamp(100 * U.ms, 20 * U.ms, 3 * U.nA, frequency=7 * U.kHz)
        self.assertEqual(7, clamp.frequency)
        self.assertEqual([(100, 3), (120, 3), (120, 0)], clamp.envelope)

        # Explicit envelope:
        envelope = [(1, 10), (3, 30), (5, 50), (7, 0)]
        clamp = A.i_clamp([(t * U.ms, i * U.nA) for t, i in envelope])
        self.assertEqual(0, clamp.frequency)
