...
class recipe(A.recipe):
    ...
    def event_generators(self, gid):
        return [A.event_generator(f"syn_{i}", 1., A.explicit_schedule(np.array([0])*U.ms)) for i in self.stim]
...
class TestDiffusionScaling(unittest.TestCase):
    ...
    def __init__(self, args):
        ...
        self.inject_max_amount = 1.0  # maximum (initially injected) particle amount in µmol/l / 1e-21 mol
    ...
    def simulate_arbor(self, mech_name):
        ...
        for i in range(self.Nx):
            ...
            if i == 0:
                locals()["dendrite_0"] = tree.append(A.mnpos,
                            A.mpoint(-self.L/2, 0, 0, self.dendrite_radius),
                            A.mpoint(-self.L/2+self.dx, 0, 0, self.dendrite_radius), tag=0)
                labels["dendrite_0"] = "(tag 0)"
            # all following segments
            else:
                locals()[f"dendrite_{i}"] = tree.append(locals()[f"dendrite_{i-1}"],
                            A.mpoint(-self.L/2+i*self.dx, 0, 0, self.dendrite_radius),
                            A.mpoint(-self.L/2+(i+1)*self.dx, 0, 0, self.dendrite_radius), tag=i)
        ...
        for i in self.stim:
            mech_inject = A.mechanism(f"{mech_name}/x=my_ion", {"alpha": self.inject_max_amount})
            decor.place(f'(on-components 0.5 (region "dendrite_{i}"))',
                        A.synapse(mech_inject), f"syn_{i}")
    ...
    def test_diffusion_scaling_amount(
        ...
        self.assertTrue(np.allclose(
                data_arbor[0],
                data_ind[0],
                atol=self.dev_ss*self.inject_max_amount
        ...
        self.assertTrue(np.allclose(
                data_arbor,
                data_ind,
                atol=self.dev_dyn*self.inject_max_amount
    ...
    @fixtures.single_context()
    def test_diffusion_scaling_concentration(
        self, single_context
    ):
        # perform the simulations
