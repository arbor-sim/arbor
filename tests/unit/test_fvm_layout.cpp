...
TEST(fvm_layout, ion_weights) {
    ...
    for (auto run: count_along(mech_segs)) {
        ...
        // TODO: move this test to fvm lowered test!
/*
...

    {
        ...
        ASSERT_EQ(1u, M.ions.count(ionKind::ca));
        ASSERT_EQ(1u, M.mechanisms.count("test_kinlva"));
        ASSERT_EQ(1u, M.mechanisms.count("test_ca"));

        auto& ca = M.ions.at(ionKind::ca);
        auto& test_kinlva = M.mechanisms.at("test_kinlva");
        auto& test_ca = M.mechanisms.at("test_ca");

        EXPECT_EQ(uvec({1, 2, 3}), ca.cv);
        EXPECT_EQ(fvec({0.75, 1., 0}), ca.iconc_norm_area);
