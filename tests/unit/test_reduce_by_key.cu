...
template <typename T>
void reduce(const std::vector<T>& in, std::vector<T>& out, const std::vector<int>& index, unsigned block_dim=128) {
    ...
    array  dst = memory::make_const_view(out);;
