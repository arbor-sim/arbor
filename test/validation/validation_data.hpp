...
public:
    ...
    std::map<std::string, trace_data<double>> load_traces(const anc::path& name);
    ...
    void set_datadir(const anc::path& dir) { datadir_ = dir; }

    void set_output(const anc::path& file) {
        out_.open(file);
        if (!out_) {
