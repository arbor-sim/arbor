...
struct linear: public ::arb::recipe {
    linear(double x, double d, double c): extent{x}, diameter{d}, cv_length{c} {
        ...
        gprop.default_parameters.discretization = arb::cv_policy_max_extent_um(cv_length);
        // Stick morphology
        // -----x-----
        segment_tree tree;
        auto p = mnpos;
...
TEST(diffusion, errors) {
    {
        ...
        ASSERT_THROW((void)run_cell(rec, {}), illegal_diffusive_mechanism);
    }
    {
        // Cannot R/W Xd w/o setting diffusivity
    ...
    }
    {
        ...
        ASSERT_THROW((void)run_cell(rec, {}), cable_cell_error);
    }
    {
        // No negative diffusivity
...
TEST(diffusion, elided_arrays) {
    ...
    EXPECT_NO_THROW((void)run_cell(rec, exp, "nai"));
    EXPECT_NO_THROW((void)run_cell(rec, exp, "nao"));
    EXPECT_NO_THROW((void)run_cell(rec, exp, "nad"));
