    ...
    std::vector<testcase> testcases = {
        {
            ...
            "pp_.minf[i_] = 1.0-1.0/(1.0+exp((v-k)/k));\n"
            "pp_.hinf[i_] = 1.0/(1.0+exp((v-k)/k));\n"
            "pp_.mtau[i_] = 0.5;\n"
            "pp_.htau[i_] = 1500.0;\n"
        }
    };
...
TEST(CPrinter, proc_body_inlined) {
    const char* expected =
        ...
        "r_7_ = 0.;\n"
        "r_8_ = 0.;\n"
        "r_9_=pp_.s2[i_]*0.33333333333333331;\n"
        "r_8_=pp_.s1[i_]+2.0;\n"
        "if(pp_.s1[i_]==3.0){\n"
        "   r_7_=2.0*r_8_;\n"
        "}\n"
        ...
        "   else{\n"
        "       r_10_=exp(r_8_);\n"
        "       r_7_=r_10_*pp_.s1[i_];\n"
        "   }\n"
        "}\n"
        "r_13_=0.;\n"
        "r_14_=0.;\n"
        "r_14_=r_9_/pp_.s2[i_];\n"
        "r_15_=log(r_14_);\n"
        "r_13_=42.0*r_15_;\n"
        "r_6_=r_9_*r_13_;\n"
        "t0=r_7_*r_6_;\n"
...
TEST(SimdPrinter, simd_if_else) {
    std::vector<const char*> expected_procs = {
            ...
            "indirect(pp_.s+i_, simd_width_) = S::where(S::logical_not(mask_0_),simd_cast<simd_value>((double)42.0));\n"
            "indirect(pp_.s+i_, simd_width_) = u;"
            ,
            "simd_value u;\n"
            "simd_mask mask_1_ = S::cmp_gt(i, (double)2.0);\n"
            "S::where(mask_1_,u) = (double)7.0;\n"
            "S::where(S::logical_not(mask_1_),u) = (double)5.0;\n"
            "indirect(pp_.s+i_, simd_width_) = S::where(S::logical_and(S::logical_not(mask_1_), mask_input_),simd_cast<simd_value>((double)42.0));\n"
            "indirect(pp_.s+i_, simd_width_) = S::where(mask_input_, u);"
            ,
            "simd_mask mask_2_ = S::cmp_gt(simd_cast<simd_value>(indirect(pp_.g+i_, simd_width_)), (double)2.0);\n"
            "simd_mask mask_3_ = S::cmp_gt(simd_cast<simd_value>(indirect(pp_.g+i_, simd_width_)), (double)3.0);\n"
            "S::where(S::logical_and(mask_2_,mask_3_),i) = (double)0.;\n"
            "S::where(S::logical_and(mask_2_,S::logical_not(mask_3_)),i) = (double)1.0;\n"
            "simd_mask mask_4_ = S::cmp_lt(simd_cast<simd_value>(indirect(pp_.g+i_, simd_width_)), (double)1.0);\n"
            "indirect(pp_.s+i_, simd_width_) = S::where(S::logical_and(S::logical_not(mask_2_),mask_4_),simd_cast<simd_value>((double)2.0));\n"
