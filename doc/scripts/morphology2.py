...
def translate(points, f, xshift):
    return [(f*p[0]+xshift, -f*p[1]) for p in points]
                ...
                for sec in branch.sections:
                    for seg in sec:
                        if seg.length>0.00001: # only draw nonzero length segments
                            line = translate(seg.corners(), sc, offset)
                            lines.add(dwg.polygon(points=line, fill=tag_colors[seg.tag]))

            elif method=='branches':
                for line in branch.outline():
                    lines.add(dwg.polygon(points=translate(line, sc, offset), fill=branchfillcolor))

                pos = translate([branch.location(0.5)], sc, offset)[0]
...
def label_image(morphology, labels, filename, sc=20):
    ...
    for branch in morph:
        for sec in branch:
            sec['x'] = [ sc*x for x in sec['x']]
            sec['y'] = [-sc*x for x in sec['y']]
            sec['r'] = [ sc*x for x in sec['r']]
...
def generate(path=''):
    ...
    #morph_image([trees.morphlab], ['branches'], path+'/morphlab.svg')
    morph_image([trees.morphlab, trees.morphlab], ['segments', 'branches'], path+'/morphlab.svg')
    print('------------------------  done  -----------------------')
