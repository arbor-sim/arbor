...
Cable cell probe addresses are definied analagously to their counterparts in
...
one, or more probes, one per site.
...
Membrane voltage
   .. py:function:: cable_probe_membrane_voltage(locations)

   Cell membrane potential (mV) at the sites specified by the location
   expression string ``locations``. This value is spatially interpolated.
...
Axial current
   .. py:function:: cable_probe_axial_current(locations)

   Estimation of intracellular current (nA) in the distal direction at the
   sites specified by the location expression string ``locations``.

   Metadata: the explicit :class:`location` of the sample site.

Ionic current
   .. py:function:: cable_probe_ion_current_density(locations, ion)
   ...
   .. py:function:: cable_probe_ion_current_cell()
...
Total ionic current
   .. py:function:: cable_probe_total_ion_current_density(locations)
...
Density mechanism state variable
   .. py:function:: cable_probe_density_state(locations, mechanism, state)

   The value of the state variable ``state`` in the density mechanism ``mechanism``
   at the sites specified by the location expression ``locations``.
...
Ionic internal concentration
   .. py:function:: cable_probe_ion_int_concentration(locations, ion)
...
Ionic external concentration
   .. py:function:: cable_probe_ion_ext_concentration(locations, ion)
